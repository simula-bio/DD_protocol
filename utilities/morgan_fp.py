...
parser.add_argument(
    "-sfp",
    "--smile_folder_path",
    help="name of the folder with prepared smiles",
    required=True,
)
parser.add_argument(
    "-fn", "--folder_name", help="name of morgan fingerprint folder", required=True
)
...
def morgan_fingp(fname):
    ...
    with open(fname, "r") as ref:
        for line in ref:
            zin_id, smile = line.rstrip().split(",")
            arg = np.zeros((1,))
            try:
                ...
                ref2.write(
                    ",".join([zin_id] + [str(elem) for elem in np.where(arg == 1)[0]])
                )
                ref2.write("\n")
            except:
                print(line)
